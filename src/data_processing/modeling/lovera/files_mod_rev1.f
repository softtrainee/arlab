...
C	if(yes.ne.'y')then
C      Print *,'Do you want to exclude some points' 
C      Print *, 'from E calculation (yes/no)?'
C	read *,yesp
C	 if (yesp.eq.'y')then
C	   print *, 'How many points do you want to remove?'
C	   read *, np
C	   do 4 k=1,np
C	     Print *, 'Enter the step # of the point(s) to be removed'
C	     read *, prm(k)
C4        continue
...
C	   k2=1
C5	 continue
C	  if (prm(k0).eq.k1)then
C	   k1=k1+1
C	   k0=k0+1
C	   if(k1.le.nst)goto 5
C	 endif
C	   tinvaux(k2)=tinv(k1)
C	   xlogdaux(k2)=xlogd(k1)
C	   wtaux(k2)=wt(k1)
C	   telabaux(k2)=telab(k1)
C	   k2=k2+1
C	   k1=k1+1
C	   if(k1.le.nst)goto 5
C	   nst=k2-1
