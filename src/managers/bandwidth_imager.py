...
from traits.api import HasTraits, Int, Enum, File, Instance, Button, Float, Str, on_trait_change, Bool
from traitsui.api import View, Item, VGroup, HGroup, RangeEditor, EnumEditor, spring
...
#        im = Image.fromarray(im)
    ...
    def _histogram_equalize_changed(self):
        ...
        if self.histogram_equalize:
            plot = self.plot
            src = self._ndim
            self._hdim = hdim = equalize(src) * 255
...
#            self._load_image(self.path)
        ...
        if self.contrast_equalize:
            plot = self.plot
            src = self._ndim
            img_rescale = self._contrast_equalize(src)
...
#
    ...
    def _contrast_equalize(self, src):
        ...
        p98 = percentile(src, self.contrast_high)
        img_rescale = rescale_intensity(src,
                                        in_range=(p2, p98)
                                        )
        return img_rescale
